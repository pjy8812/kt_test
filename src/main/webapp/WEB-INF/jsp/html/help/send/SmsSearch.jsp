...
<div class="card-box search-box">
	...
	<div class="row">
		<div class="col-10">
			<div class="row">
				...
				<div class="col-3">
					<div class="form-group row mb-3">
						<label for="faqCategory" class="col-4 col-form-label">발송유형</label>
						<div class="col-8">
							<select class="form-control" id="sendType">
								<option value="all">유형(전체)</option>
								<c:forEach var="row" items="${sendType}">
									<option value="${row.codeId}" <c:if test="${row.codeId eq vo.sendType}">selected</c:if>>${row.codeName}</option>
								</c:forEach>
							</select>
						</div>
					</div>
	...
	<div class="row">
		...
		<div class="col-2 text-right">
			<button type="button" name="btnSmsSearch" class="btn btn-custom04 btn-rounded waves-effect waves-dark">조회</button>
		</div>
	</div>
</div>
<!-- End search -->
...
<script>
	...
	//조회버튼
	$('button[name="btnSmsSearch"]').on('click touch', function () {
		data.nowPage = 1;
		searchSmsList();
	});
