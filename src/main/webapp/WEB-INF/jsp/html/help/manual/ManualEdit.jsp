...
<!-- Begin page -->
<div class="card-box edit-box" data-manual_seq="${result.manualSeq}" data-url="${url}">
	<h5 class="card-title">
		<c:choose>
			<c:when test="${empty result.manualSeq}">매뉴얼 작성</c:when>
			<c:otherwise>매뉴얼 수정</c:otherwise>
		</c:choose>
	</h5>
		...
		<table class="table mb-0 basic-table">
			...
			<tbody>
				<tr>
					...
					<td>
						<input type="text" class="form-control" name="manualTitle" value="${result.manualTitle}"/>
					</td>
				</tr>
				<tr>
					...
					<td>
						<div class="form-check-inline">
							<input type="radio" class="form-check-input" name=manualRecipient id="typeAll" value="공통" <c:if  test="${result.manualRecipient eq '공통' or empty result.manualRecipient}">checked</c:if>>
							<label class="form-check-label" for="typeAll">공통</label>
						</div>
						<div class="form-check-inline">
							<input type="radio" class="form-check-input" name="manualRecipient" id="typeFrgown" value="화주" <c:if  test="${result.manualRecipient eq '화주'}">checked</c:if>>
							<label class="form-check-label" for="typeFrgown">화주</label>
			 			</div>
						<div class="form-check-inline">
							<input type="radio" class="form-check-input" name="manualRecipient" id="typeCarown" value="차주" <c:if  test="${result.manualRecipient eq '차주'}">checked</c:if>>
							...
							<div class="col-3">
								<select class="form-control" id="manualCategory">
									...
									<c:forEach var="row" items="${faqCategory}">
										<option value="${row.codeId}" <c:if test="${row.codeId eq result.manualCategory}">selected</c:if>>${row.codeName}</option>
									</c:forEach>
								</select>
							</div>
						</div>
				...
				<tr>
					...
					<td>
						...
						<textarea class="form-control" rows="15" name="manualContent" onkeyup="fnCheckByte(2000, this)">${result.manualContent}</textarea>
				...
				<tr>
					...
					<td>
						<div class="form-check-inline">
							<input type="checkbox" class="form-check-input" name="manualExposedAlwaysYN" <c:if test="${result.manualExposedAlwaysYn eq 'Y' or empty result.manualExposedAlwaysYn}">checked</c:if>>
							<label class="form-check-label" for="manualExposedAlwaysYN">상시게시</label>
						</div>
						<div class="row pt-1 d-none" id="divPostingPeriod">
							<div class="col-12">
								<div class="row">
								<div class="col-5">
									<div class="row">
										<div class="col-6">
											<div class="picker-wrap">
												<input type="text" id="startDate" name="startDate" class="form-control" value="<fmt:formatDate value="${result.manualExposedStartDateByLongType}" pattern="yyyy.MM.dd"/>"> 
												<label for="startDate"><i class="fas fa-calendar"></i></label>
											</div>
										</div>
										<div class="col-6">
											<input type="text" id="startTime" name="startTime" class="form-control" value="<fmt:formatDate value="${result.manualExposedStartDateByLongType}" pattern="HH:mm"/>">
										</div>
									</div>
								</div>
								...
								<div class="col-5">
									<div class="row">
										<div class="col-6">
											<div class="picker-wrap">
												<input type="text" id="endDate" name="endDate" class="form-control" value="<fmt:formatDate value="${result.manualExposedEndDateByLongType}" pattern="yyyy.MM.dd"/>"> 
												<label for="endDate"><i class="fas fa-calendar"></i></label>
											</div>
										</div>
										<div class="col-6">
											<input type="text" id="endTime" name="endTime" class="form-control" value="<fmt:formatDate value="${result.manualExposedEndDateByLongType}" pattern="HH:mm"/>">
										</div>
									</div>
								</div>
							</div>
...
<div class="row">
	...
	<div class="col-6 text-right">
		<div class="button-list">
			...
			<c:choose>
				<c:when test="${empty result.manualSeq}">
					<c:if test="${isCreate}">
...
<script>
	...
	$(document).ready(function() {
		...
		/*
		var manualContent = '${result.manualContent}'.htmlChars()
		$('#editor').html($("<div/>").html(manualContent).text());
	...
	//게시기간> 상시게시 변경되었을 경우
	$('[name="manualExposedAlwaysYN"]').change(function(){
	...
	$('#btnCancel').on('click', function(){
		if(!confirm('매뉴얼 작성을 취소하시겠습니까?')){	//아니오 클릭시 return
			return; 
		}
	...
	$('#btnSave').on('click', function(){
		if(!confirm('매뉴얼 저장하시겠습니까?')){	//아니오 클릭시 return
			return;
		} 
