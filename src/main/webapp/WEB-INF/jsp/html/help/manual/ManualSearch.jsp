					...
					<div class="form-group row mb-3">
						...
						<div class="col-8 col-form-label text-left">
							<div class="form-check-inline">
								<input type="checkbox" class="form-check-input" name="manualRecipient" id="typeAll" value="all" <c:if test="${empty vo.manualRecipient or fn:indexOf(vo.manualRecipient, 'all') > -1}">checked</c:if>>
								<label class="form-check-label" for="typeAll">공통</label>
							</div>
							<div class="form-check-inline">
								<input type="checkbox" class="form-check-input" name="manualRecipient" id="typeFrgown" value="freightOwner" <c:if test="${empty vo.manualRecipient or fn:indexOf(vo.manualRecipient, 'freightOwner') > -1}">checked</c:if>>
								<label class="form-check-label" for="typeFrgown">화주</label>
				 			</div>
							<div class="form-check-inline">
								<input type="checkbox" class="form-check-input" name="manualRecipient" id="typeCarown" value="carOwner" <c:if test="${empty vo.manualRecipient or fn:indexOf(vo.manualRecipient, 'carOwner') > -1}">checked</c:if>>
						...
						<div class="col-8">
							<select class="form-control" id="manualCategory">
								...
								<c:forEach var="row" items="${faqCategory}">
									<option value="${row.codeId}" <c:if test="${row.codeId eq vo.manualCategory}">selected</c:if>>${row.codeName}</option>
								</c:forEach>
							</select>
						</div>
					</div>
...
<script>
	<c:if test="${null ne vo.manualExposedStartDate and '' ne vo.manualExposedStartDate}">
		data.startDate =  new Date(new Date(${vo.manualExposedStartDate}).setHours(0, 0, 0, 0)) 
	</c:if>
	<c:if test="${null ne vo.manualExposedEndDate and '' ne vo.manualExposedEndDate}">
		data.endDate = new Date(new Date(${vo.manualExposedEndDate}).setHours(23, 59, 59, 0));
	</c:if>
