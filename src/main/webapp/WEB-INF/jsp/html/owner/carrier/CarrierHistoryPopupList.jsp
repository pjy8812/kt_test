  	 		...
  	 		<c:forEach var="row" items="${result.list}">
  	 			<tr role="row">
		            <td>
		           	...
		           	<td>
		           		<span class="<c:choose><c:when test="${row.carrierHistoryCarrierMemberStatus eq 'normal'}">color-status-complete</c:when><c:when test="${row.carOwnerMemberStatus eq 'suspension'}">color-status-cancel</c:when><c:when test="${row.carOwnerMemberStatus eq 'withdrawalRequested' or row.carOwnerMemberStatus eq 'withdrawal'}">color-status-ing</c:when></c:choose>">
							<c:forEach var="memberRow" items="${memberStatus}">
								<c:if test="${memberRow.codeId eq row.carrierHistoryCarrierMemberStatus}">${memberRow.codeName}</c:if>
							</c:forEach>
						</span>
		           	</td>
		            <td>${row.carrierHistoryMemo}</td>
		            <td>${row.carrierHistoryAmender}</td>
		            <td>
		            	<fmt:formatDate value="${row.carrierHistoryAmendDate}" pattern="yyyy.MM.dd"/><br/>
		           	 	<fmt:formatDate value="${row.carrierHistoryAmendDate}" pattern="HH:mm"/>
		            </td>
		        </tr>
  	 		</c:forEach>
   		</tbody>
