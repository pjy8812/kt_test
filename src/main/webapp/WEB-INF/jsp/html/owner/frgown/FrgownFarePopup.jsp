...
<div id="frgownFarePopup" class="modal fade" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-full-width modal-dialog-scrollable">
        <div class="modal-content" style="width:900px">
            ...
            <div class="modal-body">
            	<div class="card-box info-box" data-freight-owner-id="${info.freightOwnerId}" data-freight-owner-fare-code="${info.freightOwnerFareCode}">
					<div class="row">
						<div class="col-6">
										...
										<tbody>
											...
											<c:forEach var="row" items="${result.list}" varStatus="i">
												<tr role="row" data-fare-code="${row.freightOwnerFareCode}" data-fare-code-name="${row.freightOwnerFareCodeName}" data-fare-apply-date="${row.freightOwnerFareApplyDate}">
													...
													<td>
														<c:choose>
															<c:when test="${empty row.freightOwnerBasicYn}">-</c:when>
															<c:otherwise>${row.freightOwnerBasicYn}</c:otherwise>
														</c:choose>
													</td>
													<td>
														<c:choose>
													...
													<td>
														<c:choose>
															<c:when test="${empty row.freightOwnerFareModifier}">-</c:when>
															<c:otherwise>${row.freightOwnerFareModifier}</c:otherwise>
														</c:choose>
													</td>
													<td>
														<c:choose>
															<c:when test="${empty row.freightOwnerFareModifyDate}">-</c:when>
															<c:otherwise>${row.freightOwnerFareModifyDate}</c:otherwise>
														</c:choose>
													</td>
													<td>
														<c:if test="${row.fareCodeCheck eq 'Y'}">
															<button type="button" class="btn btn-white waves-effect waves-dark btnChoice"><span id="choiceValue" class="text">선택취소</span></button>
														</c:if>
														<c:if test="${row.fareCodeCheck eq 'N'}">
...
<script>
	...
	$('button[name="saveBtn"]').on('click touch', function (e) {
		...
		$('#dataGrid>tbody>tr').each(function(){
			...
			var choiceText = td.eq(6).children().text();
		...
		if (saveList.length > 1) {
			...
			if(confirm('요금제 정보 변경을 진행하시겠습니까?')) {
				...
				frgownFareUpdate(_id, data.object);					
			}
		}
	});	
	
	//조회
	function searchFareList(){
		var object = {}; 
		...
		$.ajax({
			type : "POST",
			url : "<c:url value='/ui/service/owner/frgown/popup/fare.select/'/>"+ object,
			data : object,
			cache : false,
			async : true,
			beforeSend: function( xhr ) {
				$('#frgownFareModal').empty();
			},
	...
	//페이지네이션 
	$('#frgownFareModal ul.pagination li').on('click touch', function() {
	...
	$('#btnClose').on('click', function (e) {
		e.stopPropagation();
		$('#frgownFarePopup').modal('hide');
	});
	
	$('#frgownFareModal').on('hidden.bs.modal', function () {
