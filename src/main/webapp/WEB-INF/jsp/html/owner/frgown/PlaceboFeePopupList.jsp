  	 		...
  	 		<c:forEach var="row" items="${result.list}" varStatus="i">
  	 			<tr role="row">
		            <td>
		             ...
		             <td>${row.orderNum}</td>
		            <td>${row.freightOwnerPenaltyPolicyActicity}</td>
		            <td>${row.freightOwnerPenaltyPolicyDetail}</td>
		            <td>
		            	${row.freightOwnerPenaltyPolicyMinT}~${row.freightOwnerPenaltyPolicyMaxT}톤: 
		            		<c:if test="${not empty row.freightOwnerPenaltyPolicyPenalty}">
								<fmt:formatNumber value="${row.freightOwnerPenaltyPolicyPenalty}" pattern="#,###"/>원
							</c:if>
		            </td>
		            <td>
